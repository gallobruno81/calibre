...
import { CompanyProfile, SurveyResponse, Intervention } from '../types/survey';
...
export const generateDemoData = () => {
  ...
  // Mixed scores to show Traffic Light system
  for (let i = 0; i < 35; i++) {
    const daysAgo = Math.floor(Math.random() * 30);
    const team = teams[Math.floor(Math.random() * teams.length)];

    // Simulate: 'Ventas' is burned out (Red), 'Ingeniería' is okay (Amber), others are good (Green)
    let baseScore = 3.5;
    ...
    responses.push({
      ...
      team: team,
      metrics: {
        emotionalLoad: Math.min(5, Math.max(1, baseScore + (Math.random() * 2 - 0.5))),
        fatigue: Math.min(5, Math.max(1, baseScore + (Math.random() * 2 - 0.5))),
        clarity: Math.min(5, Math.max(1, baseScore + 0.5 + (Math.random() * 1.5 - 0.5))), // Usually clearer
        communication: Math.min(5, Math.max(1, baseScore + (Math.random() * 2 - 0.5))),
